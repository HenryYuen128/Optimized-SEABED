...
#         CreateMultivariateEmdProperty(g)
...
#         print()
#         print('-'*10, 'simProp','-'*10)
#         print(g.edge_properties['simProp'])
#         print(np.array(g.edge_properties['simProp']).shape)
...
#     print('-'*6, 'Get SymLaplacian', '-'*6)
    ...
    degreeInvPower = np.power(degreeInv, 1/2)
    
    l = np.dot(np.dot(degreeInvPower, l), degreeInvPower)
...
def PartitionGraphAndSegment(g, parentLabel):
    ...
    lap = PerformSymLaplacian(adj) # Laplacian矩阵
...
#         if g0.num_vertices() >= minSubpopSize and g1.num_vertices() >= minSubpopSize:
        # if sscore0 > sscoreTh:
...
def saveDataListAndLabel(localResultsFolder, expTag, curTarg, sensValList, cellLineList):
    ...
    import os
    saveDataLabelFolder = 'SYMB_DataAndLabel'
